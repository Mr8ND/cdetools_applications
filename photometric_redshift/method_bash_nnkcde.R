...
if (length(args)<=1) {
  stop("Both k and h needs to be supplied.")
} else {
  k = as.integer(args[1])
  h = as.numeric(args[2])
}
...
name <- paste("NNKCDE", str_replace(as.character(h), '0.', '0-'),
              as.character(k), sep = '_')
print(name)
train <- function(x_train, y_train, k=NULL, h=NULL) {
  obj <- NNKCDE::NNKCDE$new(x_train, y_train, h=h, k=k)
...
obj <- train(x_train, y_train, k=k, h=h)
print(c(obj$h, obj$k))
